...
public class MenorCaminho {
    ...
    public static void main(String[] args) {
        ...
        try {
            Date d1 = new Date();
            grafo.loadData();
            System.out.printf("Dados Carregados: %ds\n",
                    (new Date().getTime() - d1.getTime()) / 1000);
        } catch (FileNotFoundException ex) {
            Logger.getLogger(MenorCaminho.class.getName()).log(Level.SEVERE, null, ex);
        }
        System.out.println("Descubra a menor distancia entre 2 cidades");
        System.out.println("Digite a origem (num):");
        Scanner s = new Scanner(System.in);
        int i = s.nextInt();
        Vertice origem = grafo.getVertice(i);
        grafo.dijkstra(origem);

        System.out.println("Digite o destino (num):");
        s = new Scanner(System.in);
        i = s.nextInt();
        Vertice destino = grafo.getVertice(i);
        System.out.printf("Menor distância entre %s e %s: %.2fKm\n", origem.nome,
